  ...
  implements CustomTransportStrategy
{
  ...
  private bindHandlers() {
    this.messageHandlers.forEach((handler, topicName) => {
      const { subscriptionName, errorStrategy } =
        handler.extras as HandlerExtra;
      ...
      const subscription = receiver.subscribe(
        {
          processMessage: async (
            ...
            try {
              ...
              if (!(message as any).delivery.remote_settled)
                await receiver.completeMessage(message);
            } catch (e) {
              if (errorStrategy.type === ErrorStrategies.ExponentialBackoff) {
                await this.handleExponentialBackoff(
                  topicName,
                  message,
  ...
  private async handleExponentialBackoff(
    ...
    const currentAttempts =
      (message.applicationProperties.attemptCount as number) || 0;
    ...
    if (limitReached) {
      ...
      const scheduledTime = moment()
        .add(nextAttempt, options.measure)
        .toDate()
        .getTime();
